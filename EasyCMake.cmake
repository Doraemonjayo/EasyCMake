...
function(link_package package)
    target_link_package(${target} ${package_list} ${package} ${ARGN})
    set(${package_list} ${${package_list}} PARENT_SCOPE)
    set(${package}_COMPONENTS ${${package}_COMPONENTS} PARENT_SCOPE)
    set(${target}_DIRECTORIES ${${target}_DIRECTORIES} PARENT_SCOPE)
    set(${target}_LIBRARIES ${${target}_LIBRARIES} PARENT_SCOPE)
endfunction()

macro(include_pkglinkers target package_list)
    set(target ${target})
    set(package_list ${package_list})
    set(pkglinkers ${ARGN})
    foreach(pkglinker ${pkglinkers})
        include(${pkglinker})
    endforeach()
endmacro()
